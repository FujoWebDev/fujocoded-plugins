...
// Cache the in-flight promise (not the resolved client) so concurrent callers
// for the same repo share one handle→DID→PDS lookup. Rejections evict the
// entry so the next caller retries instead of getting a permanent failure.
